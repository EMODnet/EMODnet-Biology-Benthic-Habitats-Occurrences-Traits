...
```{r}
write_csv(tibble(name = biotic_species), "data/derived_data/biotic_aphias_to_check.txt")
```
...
```
Now add functional group, using functional group matching function in `get_worms_fgrp.R` (included in scripts folder here, also available from https://github.com/tomjwebb/WoRMS-functional-groups):
```{r}
