...
```
Quick plot of the benthic sampling events:
```{r}
...
```
So around 40% of events matched to sediment values. Add these sediment values back to `benth_events`:
```{r}
benth_events <- benth_events %>% bind_cols(sed_by_event)
```

Check where the unmatched events are:
```{r}
mapview(filter(benth_events, is.na(MudPercent)))
```
Either northern (as expected) or very close to shore (e.g. coastal, intertidal, estuarine) and not well covered by the sediment dataset.
...
Note that some events now have more than one entry:
```{r}
